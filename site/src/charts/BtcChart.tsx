...
export function BtcChart({ series, allYears, range, currency, rates }: { series: YearSeries[]; allYears: number[]; range: Range | null; currency: Currency; rates: Record<string, number> }) {
  if (series.length === 0) {
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="custom-tooltip" style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 8, padding: "8px 10px", fontSize: 12, color: "var(--fg)" }}>
                    ...
                    <div style={{ color: "#f59e0b" }}>BTC: {row.indexed.toFixed(2)}%, {priceStr}</div>
                  </div>
                );
              }}
            />
